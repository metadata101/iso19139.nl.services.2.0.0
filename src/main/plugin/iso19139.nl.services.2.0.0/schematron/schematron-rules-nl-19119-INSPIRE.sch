...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2">
  ...
  <sch:pattern id="Validatie tegen het Nederlands metadata profiel op ISO 19119">
    ...
    <sch:let name="thesaurus1" value="normalize-space(/gmd:MD_Metadata/gmd:identificationInfo/*/gmd:descriptiveKeywords[1]/gmd:MD_Keywords/gmd:thesaurusName/gmd:CI_Citation/gmd:title[gco:CharacterString or gmx:Anchor]/*)"/>
    <sch:let name="thesaurus2" value="normalize-space(/gmd:MD_Metadata/gmd:identificationInfo/*/gmd:descriptiveKeywords[2]/gmd:MD_Keywords/gmd:thesaurusName/gmd:CI_Citation/gmd:title[gco:CharacterString or gmx:Anchor]/*)"/>
    <sch:let name="thesaurus3" value="normalize-space(/gmd:MD_Metadata/gmd:identificationInfo/*/gmd:descriptiveKeywords[3]/gmd:MD_Keywords/gmd:thesaurusName/gmd:CI_Citation/gmd:title[gco:CharacterString or gmx:Anchor]/*)"/>
    <sch:let name="thesaurus4" value="normalize-space(/gmd:MD_Metadata/gmd:identificationInfo/*/gmd:descriptiveKeywords[4]/gmd:MD_Keywords/gmd:thesaurusName/gmd:CI_Citation/gmd:title[gco:CharacterString or gmx:Anchor]/*)"/>
  ...
  -->
      ...
      <!-- Voor interoperabele + harmonised SDS moet deze wel gecontroleerd worden. Alleen voor services (niet voor datasets van toepassing) -->
      <sch:assert id="Code_referentiesysteem_ISO_nr_207" etf_name="Code referentiesysteem (ISO nr. 207)" test="(not($conformity_Spec_Title_SDS_Interop) and not($conformity_Spec_Title_SDS_Harmo)) or $referenceSystemInfo">Code referentiesysteem (https://docs.geostandaarden.nl/md/mdprofiel-iso19119/#codereferentiesysteem) is verplicht voor een INSPIRE interoperabele en voor een INSPIRE geharmoniseerde service, maar ontbreekt</sch:assert>
    ...
    <sch:rule id="Conformiteit_specificaties" etf_name="Conformiteit specificaties" context="//gmd:MD_Metadata/gmd:dataQualityInfo[1]/gmd:DQ_DataQuality/gmd:report/gmd:DQ_DomainConsistency/gmd:result/gmd:DQ_ConformanceResult">
      ...
      <!-- Algemene referentie naar de verordening of een specifiek type (interoperable, harmonised, invocable) met waarde en URI-->
    ...
    <sch:rule id="alle_INSPIRE_specificaties" etf_name="alle INSPIRE specificaties" context="//gmd:MD_Metadata/gmd:dataQualityInfo/gmd:DQ_DataQuality/gmd:report/gmd:DQ_DomainConsistency/gmd:result/gmd:DQ_ConformanceResult/gmd:specification/gmd:CI_Citation">
      ...
      <!--  -->
      <!-- check opsommingen per servicetype: voor sds: verordening, + tech spec + sds type . Voor netwerk servcie verordening + tech spec -->
      ...
      <!-- voor network services: check of tech specificatie voor serviceType=view woorden 'INSPIRE' en 'View' in zich heeft, servicetype=download de woorden 'INSPIRE' en 'Download' -->
      <sch:assert id="INSPIRE_NetwerkService_typen_komt_overeen" etf_name="INSPIRE Netwerldienst type komt overeen met specificatie (ISO nr. 360) " test="not($INSPIRE_network_Spec_Title_exists) or ($INSPIRE_network_Spec_Title_exists and contains(translate($all_conformity_Spec_Titles_join, $uppercase, $lowercase), $serviceType_value) )">De metadata verwijst naar de verordening voor netwerkdiensten van INSPIRE en heeft service type '<sch:value-of select="$serviceType_value"/>', maar de specificatie (ISO nr. 360) heeft geen verwijzingen naar de technische specificatie voor het opgegeven service type. (https://docs.geostandaarden.nl/md/mdprofiel-iso19119/#specificatie)</sch:assert>
