...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:variable name="wms-info-loc">
	...
	<msg id="a" xml:lang="dut"> Service informatie overnemen uit WMS/WFS/WMTS.</msg>
    <msg id="b" xml:lang="dut"> Let op titel, omschrijving, keywords, extent worden overgenomen. </msg>
  ...
  <xsl:param name="setTitle" select="'0'"/>
  <xsl:param name="setAbstract" select="'0'"/>
  <xsl:param name="setKeywords" select="'0'"/>
  <xsl:param name="wmsServiceUrl"/>
  
  <xsl:variable name="setExtent" select="geonet:parseBoolean($setExtent)"/>
  <xsl:variable name="setTitle" select="geonet:parseBoolean($setTitle)"/>
  <xsl:variable name="setAbstract" select="geonet:parseBoolean($setAbstract)"/>
  <xsl:variable name="setKeywords" select="geonet:parseBoolean($setKeywords)"/>
  ...
  <xsl:variable name="capabilitiesDoc">
    <xsl:if test="$onlineNodes">
      <xsl:copy-of select="geonet:get-wms-capabilities($wmsServiceUrl, '1.3.0')"/>
    </xsl:if>
  </xsl:variable>
